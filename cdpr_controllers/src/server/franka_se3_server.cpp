...
FrankaSE3ActionServer::FrankaSE3ActionServer(std::string name, ros::NodeHandle &nh, std::shared_ptr<RobotController::CDPRFrankaWrapper> &mu)
: ActionServerBase(name,nh,mu), as_(nh,name,false)
{
    as_.registerGoalCallback(boost::bind(&FrankaSE3ActionServer::goalCallback, this));
    as_.registerPreemptCallback(boost::bind(&FrankaSE3ActionServer::preemptCallback, this));
    as_.start();
}

void FrankaSE3ActionServer::goalCallback()
{
...
void FrankaSE3ActionServer::preemptCallback()
{
...
bool FrankaSE3ActionServer::compute(ros::Time ctime)
{
  if (!control_running_)
    return false;
...
void FrankaSE3ActionServer::signalAbort(bool is_aborted)
{
  setAborted();  
}

void FrankaSE3ActionServer::setSucceeded()
{
  as_.setSucceeded(result_);
  
  control_running_ = false;
}
void FrankaSE3ActionServer::setAborted()
{
  as_.setAborted();
  control_running_ = false;
}